...
if (!imagePath) {
  ...
  console.error(
    "Example: npx tsx scripts/test-gemini-layout.ts ./test-samples/page-001.png",
  );
  process.exit(1);
}
...
Return in this exact format:
{
  ...
  "layoutStructure": {
    ...
    console.log(
      `${hasImageDimensions ? "✅" : "❌"} Image dimensions: ${hasImageDimensions ? `${parsed.imageWidth}x${parsed.imageHeight}` : "MISSING"}`,
    );

    const hasBlocks = Array.isArray(parsed.blocks);
    console.log(
      `${hasBlocks ? "✅" : "❌"} Blocks array: ${hasBlocks ? `${parsed.blocks?.length || 0} blocks` : "MISSING"}`,
    );
    ...
    if (hasBlocks && parsed.blocks.length > 0) {
      ...
      console.log(
        `${hasBoundingBox ? "✅" : "❌"} Bounding box format: ${hasBoundingBox ? "CORRECT" : "INVALID"}`,
      );

      const hasText =
        typeof firstBlock.text === "string" && firstBlock.text.length > 0;
      console.log(
        `${hasText ? "✅" : "❌"} Text content: ${hasText ? "PRESENT" : "MISSING"}`,
      );

      const hasType = typeof firstBlock.type === "string";
      console.log(
        `${hasType ? "✅" : "❌"} Block type: ${hasType ? firstBlock.type : "MISSING"}`,
      );

      const hasReadingOrder = typeof firstBlock.readingOrder === "number";
      console.log(
        `${hasReadingOrder ? "✅" : "❌"} Reading order: ${hasReadingOrder ? "PRESENT" : "MISSING"}`,
      );
    }

    const hasLayout = parsed.layoutStructure;
    console.log(
      `${hasLayout ? "✅" : "❌"} Layout structure: ${hasLayout ? "PRESENT" : "MISSING"}`,
    );
    ...
    if (hasBlocks && parsed.blocks.length > 0) {
      ...
      parsed.blocks.slice(0, 3).forEach((block: any, i: number) => {
        ...
        console.log(
          `  BBox: x=${block.boundingBox?.x}, y=${block.boundingBox?.y}, w=${block.boundingBox?.width}, h=${block.boundingBox?.height}`,
        );
        console.log(
          `  Text: ${block.text?.substring(0, 60)}${block.text?.length > 60 ? "..." : ""}`,
        );
...
async function main() {
  ...
  console.log(
    `✅ Multi-Column: ${results.multiColumn.success ? "PASS" : "FAIL"}`,
  );
  console.log(
    `✅ Language Detection: ${results.languageDetection.success ? "PASS" : "FAIL"}`,
  );

  const allPassed =
    results.boundingBoxes.success &&
  ...
  if (allPassed) {
    ...
    console.log(
      "   💰 Cost: ~$0.27 per 700-page book (26x cheaper than Document AI)",
    );
  } else {
